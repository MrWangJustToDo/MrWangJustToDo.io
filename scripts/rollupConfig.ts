...
const tsConfig = (relativePath: string) =>
  typescript({
    ...
    tsconfigDefaults: {
      compilerOptions: {
        rootDir: resolve(relativePath, "src"),
        composite: true,
        declaration: true,
        declarationMap: true,
        declarationDir: resolve(relativePath, "dist/types"),
      },
      include: [resolve(relativePath, "src")],
...
const transformBuildOptions = (
  ...
  packageFileObject: Record<string, unknown>,
  relativePath: string,
  mode: Mode,
): {
  ...
  if (options.output) {
    ...
    if (singleOtherConfig.length) {
      allOptions.singleOther = {
        ...
        external: (id) => id.includes("node_modules") || id.includes("@blog/"),
        plugins: [
          nodeResolve(),
