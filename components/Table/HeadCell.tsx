...
export function HeadCell<T>({
  ...
  const getColor = useCallback(
    (targetOrder: SortOrder) =>
      sorter?.by === dataIndex && sorter?.order === targetOrder
        ? sortedColor
        : undefined,
  ...
  if (typeof CustomRender === "function") {
    ...
    if (dataIndex) {
      ...
      const CustomRenderWithoutDataIndex = CustomRender as Required<
        HeadCellProps<T>
      >["CustomRender"];
  ...
  const content =
    typeof Render === "function"
      ? Render({ dataIndex: dataIndex || ("" as keyof T), rowIndex, colIndex })
      : Render;
  ...
  return (
    <Th textTransform="none" color="inherit" {...cellProps}>
      {tooltipProps ? (
        <Tooltip {...tooltipProps}>{thContent}</Tooltip>
      ) : (
        thContent
      )}
    </Th>
